...
static void sprd_sdhci_reset(struct sdhci_host *host, u8 mask)
{
...
static void sprd_sdhci_host_card_event(struct mmc_host *mmc) {
	...
	if (host->mrq && value) {
		...
		sprd_sdhci_reset(host, SDHCI_RESET_CMD);
		sprd_sdhci_reset(host, SDHCI_RESET_DATA);
...
static void sprd_sdhci_host_of_parse(struct platform_device *pdev, struct sdhci_host *host) {
	...
	if (of_find_property(np, "sdhci-cap2-poweroff-notify", &len))
		host_pdata->caps2 |= MMC_CAP2_POWEROFF_NOTIFY;
	if (of_find_property(np, "sdhci-cap2-no-sleep-cmd", &len))
		host_pdata->caps2 |= MMC_CAP2_NO_SLEEP_CMD;
	if (of_find_property(np, "sdhci-cap2-power-always-on", &len))
		host_pdata->caps2 |= MMC_CAP2_POWER_ALWAYS_ON;
